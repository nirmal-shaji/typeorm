...
import type { WhereClauseCondition } from "./WhereClause"
    ...
    implements WhereExpressionBuilder
{
    ...
    /**
     * Whether this query already joins and selects the given relation of the
     * main alias in a way that hydrates it exactly as a separate load would.
     *
     * Only an unrestricted, selected LEFT join qualifies. An INNER join, an
     * extra "ON" condition or a filter naming the joined alias all hydrate a
     * subset of the relation. A plain join also cannot reproduce the related
     * entity's own eager relations, nor a "select", "order" or "relations"
     * given for the relation in the find options.
     *
     * @param relation
     */
    protected isRelationHydratedByJoin(relation: RelationMetadata): boolean {
        if (!this.expressionMap.mainAlias) return false

        if (relation.inverseEntityMetadata.eagerRelations.length > 0)
            return false

        const findOptionsDescribeRelation = (
            source: ObjectLiteral | undefined,
        ) => {
            let value: unknown = source
            for (const property of relation.propertyPath.split(".")) {
                if (value === null || value === undefined) return false
                value = (value as ObjectLiteral)[property]
            }
            return value !== undefined
        }
        if (
            findOptionsDescribeRelation(this.findOptions.select) ||
            findOptionsDescribeRelation(this.findOptions.order) ||
            findOptionsDescribeRelation(this.findOptions.relations)
        )
            return false

        const relationPath = `${this.expressionMap.mainAlias.name}.${relation.propertyPath}`
        return this.expressionMap.joinAttributes.some(
            (join) =>
                ...
                !this.isAliasUsedInFilters(join.alias.name),
        )
    }

    /**
     * Whether any WHERE or HAVING clause names the given alias.
     *
     * @param aliasName
     */
    protected isAliasUsedInFilters(aliasName: string): boolean {
        const namesAlias = (condition: WhereClauseCondition): boolean => {
            if (typeof condition === "string")
                return condition.includes(aliasName)

            if (Array.isArray(condition))
                return condition.some((clause) => namesAlias(clause.condition))

            if ("parameters" in condition)
                return condition.parameters.some((parameter) =>
                    parameter.includes(aliasName),
                )

            return namesAlias(condition.condition)
        }

        return [
            ...this.expressionMap.wheres,
            ...this.expressionMap.havings,
        ].some((clause) => namesAlias(clause.condition))
    }
    ...
    protected async executeEntitiesAndRawResults(
        ...
        if (this.expressionMap.relationLoadStrategy === "query") {
            ...
            const loadRelation = async (
                ...
                // The main query has already hydrated this relation, and this
                // load would overwrite it with the same value.
                if (this.isRelationHydratedByJoin(relation)) return
